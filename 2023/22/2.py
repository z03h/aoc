...
def nccollide(xs1, xe1, ys1, ye1, xs2, xe2, ys2, ye2):
    return (
        (
    ...
    for state in current_bricks:
        ...
        for lower_z in range(zs-1, 0, -1):
            ...
            for (xs2, ys2, xe2, ye2, zs2, ze2, brick_id2) in top_bricks[lower_z]:
                if nccollide(xs2, xe2, ys2, ye2, xs, xe, ys, ye):
                    new_zs = ze2 + 1
                    break
...
for z_level in range(hz, 1, -1):
    ...
    for cstate in current_bricks:
        cx1, cy1, cx2, cy2, cz1, cz2, cid = cstate
        ...
        if len(collisions) == 1:
            # would cause something to fall
            to_remove |= collisions


def move_down(bottom_bricks, ignore_id) -> int:
    moved = 0
    bottom_bricks = {k: v for k, v in sorted(bottom_bricks.items())}
    ...
    for k, current_bricks in bottom_bricks.items():
        ...
        for state in current_bricks:
            ...
            for lower_z in range(zs-1, 0, -1):
                ...
                for (xs2, ys2, xe2, ye2, zs2, ze2, brick_id2) in top_bricks[lower_z]:
                    if brick_id2 != ignore_id and nccollide(xs2, xe2, ys2, ye2, xs, xe, ys, ye):
                        new_zs = ze2 + 1
                        break
...
x = sum(move_down(fallen_bricks, i) for i in to_remove)
print(x)