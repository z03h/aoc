...
def find(srow, scol, direction):
    charged = defaultdict(set)
    lights = [Light(srow, scol, direction)]
...
for i in range(0, max_cols):
    starts.append(((-1, i), DOWN))
    starts.append(((max_rows, i), UP))

# left/right side
for i in range(0, max_rows):
    starts.append(((i, -1), RIGHT))
    starts.append(((i, max_cols), LEFT))

for start in starts:
    (r, c), direction = start
    m = find(r, c, direction)
    #print((r,c), direction, m)
    if m > highest:
        highest = m
print(highest)