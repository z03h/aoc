...
while True:
    ...
    for bag in bags:
        ...
        for match in f:
            if match[1] == 'shiny gold' or match[1] in can_hold_shiny_gold or match[1] in new_add:
                print('match: ', match)
                nbag = contain[:-4].strip()
                if nbag not in can_hold_shiny_gold:
                    new_add.add(nbag)
                break
    if not new_add:
        print('empty new add')
        break
    can_hold_shiny_gold |= new_add
print(len(can_hold_shiny_gold))
