bags = '''light plum bags contain 1 faded blue bag.
...
bag_re = re.compile(r"(\d+) (.+? .+?) bag[s]?")
bags = bags.split('\n')
num_bags = 0

all_bags = defaultdict(list)
#create mapping
for bag in bags:
    contain, _, contains = bag.partition(' contain ')
    f = bag_re.findall(contains)
    nbag = contain[:-4].strip()
    if not f:
        all_bags[nbag] = []
        continue
    for match in f:
        all_bags[nbag].append((int(match[0]), match[1]))

def rec_get_count(baglist):
    c = 0
    for count, bag in baglist:
        c += count
        try:
            print('getting', bag)
            c += count * rec_get_count(all_bags[bag])
        except KeyError:
            print('keyerror')
            pass
    print('\tdone', baglist)
    return c
print(rec_get_count(all_bags['shiny gold']))
