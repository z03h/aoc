...
codes = '''aaaaaababababbbbbababaab
...
for r in rules.splitlines():
    num,_, rule = r.partition(': ')
    if '"' in rule:
        #a or b
        numrules[num] = rule.strip('"')
    else:
        # multiple rules
        numrules[num] = [r.split() for r in rule.split(' | ')]
#print(numrules)

@lru_cache(maxsize=None)
def getrule(num):
    num = str(num)
    if numrules[num] in ('a','b'):
        return [f'(?P<G{num}>{numrules[num]})']
    allpossible = []
    for rule in numrules[num]:
        single = []
        for inum in rule:
            if inum == num:
                x = [f'(?&G{num})']
                ...
                x = getrule(inum)
            single.append(x)
        allpossible.extend(''.join(a) for a in product(*single))
    xx = [f'(?P<G{num}>'+ '|'.join(allpossible) + ')']
    return xx
reeeeeee = (getrule(0)[0])
print(sum(1 for poo in codes if re.fullmatch(reeeeeee, poo)))
