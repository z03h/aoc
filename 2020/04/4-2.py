lines = '''hcl:#6b5442 ecl:brn iyr:2019
...
inputs = "byr: iyr: eyr: hgt: hcl: ecl: pid:".split()
def validpp(input):
    return all(i in input for i in inputs)
...
checks={
    ...
    'hcl:': lambda x: x[0] == '#' and all(c in '0912345678abcdef' for c in x[1:]),
    'ecl:': lambda x: x in ('amb','blu', 'brn', 'gry', 'grn', 'hzl', 'oth'),
    'pid:': lambda x: len(x) == 9 and x.isdigit()
}
def validppinput(input):
    items = input.split(' ')
    for att in items:
        if items:
            try:
                if not checks[att[:4]](att[4:]):
                    return False
            except KeyError:
                continue
            except:
...
while ilines:
    try:
        ...
        while line:
            ...
            line = next(ilines)
        text = ' '.join(inside)
        if validpp(text) and validppinput(text):
            valid += 1
