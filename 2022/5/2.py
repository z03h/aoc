...
def parse_input_buckets(starting_data) -> list[list[str]]:
    *data, last_line = starting_data.split('\n')
    ...
    final_buckets = [list(bucket) for bucket in buckets]

    return final_buckets


def handle_movment(buckets: list[list[str]], num_crates: int, from_index: int, to_index: int):
    from_bucket = buckets[from_index]
    to_bucket = buckets[to_index]
    to_bucket.extend(from_bucket[-num_crates:])
